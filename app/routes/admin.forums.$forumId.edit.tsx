...
export function meta() {
  return [{ title: "Edit forum · Admin · KuroBB" }];
}
...
export default function EditForum({ loaderData, actionData }: Route.ComponentProps) {
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col gap-8 p-6 md:p-8">
      <PageHeading>Edit forum</PageHeading>
      <Form method="post" className="flex flex-col gap-4">
        ...
        <Field name="position" type="number" label="Position" defaultValue={forum.position} />
      ...
      <Form method="post">
        ...
        <Button type="submit" variant="destructive" disabled={forum.threadCount > 0}>
          Delete forum{forum.threadCount > 0 ? ` (has ${forum.threadCount} threads)` : ""}
        </Button>
      </Form>
    </main>
  );
