...
export function meta() {
  return [
    { title: "Forums · KuroBB" },
    { name: "description", content: "Browse KuroBB's forums." },
  ];
}
        ...
        <li key={node.id} className="border-b border-border py-3 last:border-b-0">
          <div className="flex items-baseline justify-between gap-4">
            ...
            <span className="shrink-0 text-sm text-ink-muted">
              {node.threadCount} thread{node.threadCount === 1 ? "" : "s"}
            </span>
          </div>
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-8 p-6 md:p-8">
      <div className="flex items-baseline justify-between">
        <PageHeading>Forums</PageHeading>
        {!isEmpty && (
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center gap-3 py-12 text-center">
          <p className="text-ink-muted">No forums yet.</p>
